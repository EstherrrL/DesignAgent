...
def _check_syntax(code: str, language: str) -> Optional[str]:
    """
    本地静态语法检查（目前支持 Python）。
    返回 None 表示语法正确；否则返回错误描述字符串。
    这一步不消耗 LLM 调用，能在第一时间拦截语法错误的代码，
    避免 Reviewer 仅凭"感觉"给语法都不通过的代码打高分。
    """
    ...
    try:
        ...
        return f"第 {e.lineno} 行语法错误：{e.msg}（{e.text.strip() if e.text else ''}）"
...
def review_code(
    ...
    """
    # ── 第一步：本地语法预检（零 LLM 成本，拦截明显语法错误）───────────────────
    syntax_error = _check_syntax(code_result.code, code_result.language)
    if syntax_error:
        logger.warning(f"[review_code] 语法预检失败：{syntax_error}")
        return ReviewResult(
            ...
            issues=[
                Issue(
                    ...
                    description=f"代码存在语法错误，无法通过编译：{syntax_error}",
                )
            ],
            suggestions=["修复语法错误，确保代码可以正常编译/解析后再提交"],
            summary="代码未通过本地语法检查，直接判定为不通过。",
            iteration=iteration,
        )
